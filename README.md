...
performed 5 steps:
	...
	5. From the data set in step 4, creates a second, independent tidy data set with the average of each variable for eac activity and each subject.
...
The test and train subdirectories contain the x_test.txt, y_test.txt, and subject_test.txt, and the x_train.txt, y_train.txt, and subject_train.txt files respectively.  Other files used by the sctipt are in the paraent UCI HAR Dataset.
...
As just described, the full data set had been randomly divided by the researchers into training and test data sets. This was done as part of the effort to create a statistical model that could identify an action from the the only feature information.
 ...
 observations data frame containing all 10299 observation row and 561 feature attribute columns.
 ...
 **** Step 2: Extract obnly the measurements on the mean and standard ddeviation for each
 ...
 A regular expression was used with grep to extract only feature with both mean and standard deviations values.  There were 66 features that matched the regular expression.  extracted.	These included features that included mean() or std() in their names. Grep was run 2 times, once to obtain a vector of column numbers to extract, and once to extract the column names of the selected features.
...
**** Step 3: Use descriptive activity names to name the activities in the data set
 ...
 the y_train and y_test data were merged into a single data frame. The resulting daa frame was named activty_linkage. The names and number of the 6 activities wdere contained in the activity_lables.txt file, which was loaded into the activity_names data frame.
 ...
 observtional data. First, we need to merge the subject training and testing data sets
